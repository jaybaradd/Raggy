...
def retrieve_memories(query: str, *, owner_id: str = "default", session_id: str,
                      project_id: str | None = None, project_scope: str | None = None, top_k: int = 5) -> MemoryRetrievalResult:
    hits = qdrant_store.search_memories(
        query_vector=embedder.encode_query(query), query_text=query, owner_id=owner_id,
        session_id=session_id, project_id=project_id, project_scope=project_scope, top_k=top_k,
    )
    if not hits:

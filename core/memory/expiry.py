...
from core.storage.memory_store import MemoryStore, memory_store

logger = logging.getLogger(__name__)


async def run_expiry_sweep(*, store: MemoryStore = memory_store) -> int:
    """Expire due records, then synchronize their derived projections."""
    expired = await asyncio.to_thread(store.expire_due)
    if expired:
        result = await asyncio.to_thread(sync_pending_projections, store=store)
        logger.info("Expiry sweep marked %d memories expired; projections=%s", len(expired), result)
    return len(expired)


async def expiry_sweep_loop(*, run_once: Callable[[], Awaitable[int]] = run_expiry_sweep) -> None:

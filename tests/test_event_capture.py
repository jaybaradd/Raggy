...
class EventCaptureTests(unittest.TestCase):
    ...
    def _shipment(self, *, project: str = "imports", temporal: str = "Friday") -> MemoryRecord:
        return MemoryRecord(
            owner_id="user-1", scope="project", project_scope=project, kind="event",
            status="active", user_confirmed=True, source_turn_id=f"turn-{temporal}",
            payload=EventMemory(event_type="shipment", summary="Cargo shipment is arriving",
                                entities=["Shipment AC-42", "Tokyo supplier"], locations=["Tokyo"],
    ...
    def test_identical_event_is_audited_without_creating_a_second_memory(self) -> None:
        ...
        duplicate = self.store.capture_event(self._shipment(), event_type="auto_promoted_project_event")
